...
class PickAndPlace():
    def __init__(self, shape, desired_position, limb, hover_distance = 0.15, verbose=False):
		...
		self.circle_x= -0.055
                self.circle_y= 0.015
		self.triangle_x=-0.05
                self.triangle_y= 0.015
		self.square_x=-0.06
		self.square_y=0.01
    ...
    def move_to_hover_2(self):
		...
		# approach with a pose the hover-distance above the requested pose
		hover.position.z = hover.position.z + self._hover_distance
		hover.position.y = hover.position.y + 0.3
		joint_angles = self.ik_request(hover)
		self._guarded_move_to_joint_position(joint_angles)
		self.step = self.step + 1
		rospy.sleep(1.0)
	...
	if self._shape_name == "circle":
		shift_x = self.circle_x
		shift_y = self.circle_y
        elif self._shape_name == "square":
		shift_x = self.square_x
		shift_y = self.square_y
        elif self._shape_name == "triangle":
		shift_x = self.triangle_x
		shift_y = self.triangle_y

	current_pose = self._limb.endpoint_pose()
	ik_pose = Pose()
	ik_pose.position.x = current_pose['position'].x + shift_x + adjust_value[0]
	ik_pose.position.y = current_pose['position'].y + shift_y + adjust_value[1]
